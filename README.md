...
```bash
git clone https://github.com/homecatdesign/tvc-director.git ~/.cursor/skills/tvc-director
```

### Claude Code

```bash
git clone https://github.com/homecatdesign/tvc-director.git ~/.claude/skills/tvc-director
```
...
|------|---------|-------------|
| **A: Full TVC Pipeline** | "Make me a TVC for [product]" | Brief → Concept → Style → Assets → Keyframes → Package |
| **B: Quick Asset/Prompt** | "Make me a product Hero Shot" | Skip creative phase, go to asset or keyframe generation |
| **C: Storyboard** | Provide a TVC script/storyboard | Style → Assets → Convert storyboard to keyframes |
| **D: Iteration** | "This product image looks wrong" | Fix and regenerate specific assets or keyframes |
