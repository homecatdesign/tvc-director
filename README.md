...
一个将 AI Agent 转化为 **TVC 广告创意导演** 的 Skill，覆盖电视广告和品牌广告的完整创意流程：从产品 brief 到可直接使用的关键帧提示词和 Multi-Phase 视频脚本。
...
- **即拷即用** — 产出的提示词可直接粘贴到图片/视频生成工具，无需二次加工
...
### Step 2 — AI 生成产品多视图提示词 → 图片生成工具出图

AI 输出产品多视图提示词，复制到图片生成工具（edit 模式，传入参考图），得到标准化的多角度产品图：
...
### Step 4 — AI 生成 Multi-Phase 视频脚本 → 视频生成工具生成视频

AI 同步输出 Multi-Phase 视频提示词（5 Phase / 15s），配合多宫格作为首帧 + 产品多视图作为锚定，双图输入视频生成工具生成最终视频。

### 产出物一览

| 产出物 | 工具 | 用途 |
|--------|------|------|
| 产品多视图 | 图片生成工具 (edit) | 产品锚定，供后续步骤引用 |
| 9 宫格分镜图 | 图片生成工具 (edit) | 视频首帧 + 视觉校对 |
| Multi-Phase 视频脚本 | 视频生成工具 | 生成 15s 成片 |
...
│       └── ...
│
└── video-scripts/                  # Multi-Phase 视频提示词
...
1. **产品多视图** — 将 `assets/prompts/product-multiview.md` 中的提示词复制到图片生成工具，选择 edit 模式，传入参考图
2. **分镜关键帧** — 将 `keyframes/prompts/` 下的提示词复制到图片生成工具，edit 模式，传入多视图 + 环境图
3. **视频脚本** — 将 `video-scripts/` 下的 Multi-Phase 脚本配合多宫格首帧 + 产品多视图，输入视频生成工具生成视频
...
| 创意导演 | `creative.md` | 8 种 TVC 叙事模型、视觉美学设计、创意方案模板 + 2 个示例 | Phase 2（创意构思） |
| 提示词工程师 | `prompts.md` | 6 层提示词结构、画风锚定词库 A-E、12 种 TVC 场景类型、构图范式 | Phase 3-4（画风+资产） |
| 分镜与视频 | `production.md` | 多宫格分镜、视频提示词语法、产品电影化拆解、品牌世界镜头 | Phase 5（分镜生成） |
| 输出与迭代 | `infra.md` | 输出格式模板、迭代调试指南（11 种常见失败模式） | Phase 4-7（按需加载） |
