...
A skill that turns your AI agent into a **TVC Advertising Creative Director**, handling the complete creative pipeline for television commercials and brand advertising: from a product brief to production-ready keyframe prompts and Multi-Phase video scripts.
...
- **Copy-paste ready** — Output prompts work directly in image/video generation tools with no rework needed
...
### Step 2 — AI generates multiview prompts → Image generation tool renders

The AI outputs product multiview prompts. Copy to an image generation tool (edit mode, pass in the reference image) to get standardized multi-angle product shots:

<img src="https://github.com/user-attachments/assets/9d97df38-79f1-4a6a-a6bb-0fdbd731faca" width="600" alt="Product multiview" />

### Step 3 — AI generates 3×3 storyboard prompts → Image generation tool renders

The AI outputs a 3×3 multi-grid storyboard prompt (with per-panel composition, lighting, and camera direction). Copy to an image generation tool (edit mode, pass in multiview + environment images):
...
|-------------|------|---------|
| Product multiview | Image generation tool (edit) | Product anchor for subsequent steps |
| 3×3 storyboard grid | Image generation tool (edit) | Video first frame + visual QA |
| Multi-Phase video script | Video generation tool | Generate 15s final video |
...
│       └── ...
│
└── video-scripts/                  # Multi-Phase video prompts
...
1. **Product multiview** — Copy prompts from `assets/prompts/product-multiview.md` into an image generation tool, edit mode, pass in reference image
2. **Storyboard keyframes** — Copy prompts from `keyframes/prompts/` into an image generation tool, edit mode, pass in multiview + environment images
3. **Video scripts** — Use Multi-Phase scripts from `video-scripts/` with the storyboard grid as first frame + product multiview, input to a video generation tool
